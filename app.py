...
if st.session_state['logged_in']:
    ...
    show_login_page()
st.caption("© 2026 HENG ONG HUAT ANALYTICS.")        "vip_menu": "👑 VIP Exclusive",
...
def show_login_page():
    ...
    with t1:
        ...
        if st.button(L['btn_login']):
            ...
            # Smart Search: Strips invisible apostrophes, leading zeros, and ignores upper/lowercase
            found_user = None
            for db_phone, data in db.items():
                db_clean = str(db_phone).strip().lstrip("'").lstrip("0").lower()
                in_clean = clean_p.lstrip("'").lstrip("0").lower()
    ...
    with t2:
        ...
        if st.button(L['btn_reg']):
            if rn and rp and rc:
                ...
                # Check if account exists robustly
                user_exists = False
                for k in db.keys():
                    if str(k).strip().lstrip("'").lstrip("0").lower() == clean_rp.lstrip("0").lower():
                        user_exists = True
                ...
                if user_exists:
                    ...
                    # ONLY add apostrophe if it's numbers. If it's letters like 'mario', save it normally!
                    save_phone = f"'{clean_rp}" if clean_rp.isdigit() else clean_rp
                    sheet.append_row([save_phone, rc.strip(), rn.strip(), 0])
                    st.success("Registration Success! You can now log in.")
